...
start_api_service() {
    ...
    source "$VENV_DIR/bin/activate"
    cd "$PROJECT_ROOT"
    log_info "执行命令: $VENV_DIR/bin/python scripts/server/start_api.py"
    nohup "$VENV_DIR/bin/python" scripts/server/start_api.py > "$LOGS_DIR/api_service.log" 2>&1 &
    local api_pid=$!
...
stop_service() {
    ...
    # 查找所有相关进程
    local running_pids=$(pgrep -f "start_api.py\|app/main.py" 2>/dev/null)
    if [[ -n "$running_pids" ]]; then
        for pid in $running_pids; do
            # 避免重复添加
            if [[ ! " ${pids_to_stop[@]} " =~ " ${pid} " ]]; then
                pids_to_stop+=("$pid")
                log_info "发现额外的进程: $pid"
            fi
        done
    fi
    ...
    if [[ ${#remaining_pids[@]} -gt 0 ]]; then
        log_warn "以下进程未响应SIGTERM，发送SIGKILL: ${remaining_pids[*]}"
...
check_status() {
    ...
    # 检查API服务
    local api_pids=$(pgrep -f "start_api.py" 2>/dev/null)
    if [[ -n "$api_pids" ]]; then
    ...
    # 检查主应用服务
    local main_pids=$(pgrep -f "app/main.py" 2>/dev/null)
    if [[ -n "$main_pids" ]]; then
